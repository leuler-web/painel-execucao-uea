...
st.markdown("""
    <style>
    /* 1. ELIMINAR O ESPAÇO GIGANTE NO TOPO DA PÁGINA */
    .block-container {
    ...
    .tabela-customizada tbody td div[title] {
        cursor: help;
...
dict_fontes_global = {
    '201': 'Recursos Diretamente Arrecadados',
    '280': 'Convênios ou transferências',
    '116': 'Fonte do Tesouro',
    '285': 'Outras Fontes',
    '243': 'Transferências vinculadas/fundos'
}
...
# FUNÇÃO PARA A TABELA (Sem R$ e com 2 casas decimais)
                    ...
                    if col_acao:
                        ...
                        if col_nome:
                            for _, row in df_acao.iterrows():
                                ...
                                if cod.isdigit() and len(cod) == 4:
                                    dict_acoes[cod] = str(row[col_nome]).strip()
                                    
                if 'NAT' in aba.upper():
                    df_nat = df.copy()
                    if len(df_nat.columns) >= 2:
                        col_cod = df_nat.columns[0]
                        col_nome = df_nat.columns[1]
                        for _, row in df_nat.iterrows():
                            ...
                            if cod_numeros:
                                dict_naturezas[cod_numeros[:6]] = str(row[col_nome]).strip()
            
            status_msg = f"Dicionários OK! ({len(dict_acoes)} Ações)"
        except Exception as e:
            status_msg = f"Erro na leitura: {e}"
    else:
        status_msg = "Arquivo Tabelas_Auxiliares.xlsx não encontrado."
...
# --- MÁSCARA EXCLUSIVA GRÁFICO EVOLUÇÃO ---
...
if var_acao_codigo != "Todas": 
    tags.append(f"<b>🎯 Ação:</b> {var_acao_str}")
if var_fonte_codigo != "Todas": 
    tags.append(f"<b>🏦 Fonte de Recurso:</b> {var_fonte_str}")
if var_natureza_codigo != "Todas": 
    tags.append(f"<b>🏷️ Natureza da Despesa:</b> {var_natureza_str}")
...
with tab_visao:
    ...
    # =========================================================================
    # LÓGICA DA EXPLOSÃO (BAR CHART vs TREEMAP/GRÁFICO DE CALOR)
    # =========================================================================
    if var_acao_codigo == "Todas":
        ...
        if not df_top.empty:
            ...
            fig_bar.update_layout(
                yaxis=dict(categoryorder='total ascending', tickfont=dict(size=28, color="#111827")), 
        ...
        if not df_tree.empty:
            ...
            # ATUALIZAÇÃO 3.4: Removida a trava de cor para ativar o auto-contraste nativo do Plotly!
...
st.sidebar.markdown("""
    ...
    <div style='text-align: center; color: #9CA3AF; font-size: 11px; margin-top: 10px;'>
        Versão 3.4 - Auto-Contraste Inteligente 🧠
    </div>
""", unsafe_allow_html=True)
